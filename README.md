...
```
git clone https://github.com/mukunthan7/rest-prisma-express.git
```

Install npm dependencies:

```
cd rest-prisma-express
npm install
```

### 2. Create and seed the database

Run the following command to create your SQLite database file. This also creates the `User` tables that are defined in [`prisma/schema.prisma`](./prisma/schema.prisma):
...
### 4. Hard Reload to Start the Server

```
npm run dev
```

need to install nodemon in local development server
...
- `/:id`: Fetch a single data by its `id`
- `/`: Fetch a All the data
...
- `/:id`: Toggle the publish value of a post by its `id`

### `DELETE`

- `/:id`: Delete a post by its `id`
